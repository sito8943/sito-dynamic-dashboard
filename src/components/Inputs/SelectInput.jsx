...
export default function SelectInput({
  ...
  return (
    <div className={className}>
      ...
      <div
        className={`absolute z-20 p-2 w-full max-h-400 ${css({
          top: "75px",
        ...
        {loadingEffect ? (
          ...
          <div className="w-full h-full flex flex-col items-start justify-start">
            {options.length ? (
              options
                .filter((item) =>
                  list ? !list.find((jtem) => item.id === jtem.id) : true
                )
                .map((item) => (
                  <button
                    type="button"
            ...
            (!options.find(
              (item) =>
                ...
                item.name.toLowerCase() === localValue.toLowerCase()
            ) &&
              extensible) ? (
              <button
                type="button"
