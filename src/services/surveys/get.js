...
export const surveyList = async (
  ...
  localStorage.removeItem(`axios-cache:${localStorage.getItem("survey-cache")}`);
  let parameters = "";
  ...
  const response = await Axios({
    url: `${config.apiUrl}survey/list${parameters}`,
    method: "get",
    headers: {
      ...getAuth,
  ...
  // @ts-ignore
  localStorage.setItem("survey-cache", response.id);
