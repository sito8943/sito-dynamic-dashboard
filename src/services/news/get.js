...
export const newsList = async (
    ...
    if (!cleanCache)
      return JSON.parse(
        ...
        localStorage.getItem(
          `axios-cache:${localStorage.getItem("news-cache")}`
        )
      ).data;
  } catch (err) {}

  localStorage.removeItem(`axios-cache:${localStorage.getItem("news-cache")}`);
  let parameters = "";
  ...
  const response = await Axios({
    url: `${config.apiUrl}news/list${parameters}`,
    method: "get",
    headers: {
      ...getAuth,
  ...
  // @ts-ignore
  localStorage.setItem("news-cache", response.id);
